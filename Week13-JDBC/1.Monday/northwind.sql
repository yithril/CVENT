...
SELECT setval('categories_categoryid_seq', (SELECT MAX(categoryid) FROM categories) + 1);

-- Sync sequence for employees table
SELECT setval('employees_employeeid_seq', (SELECT MAX(employeeid) FROM employees) + 1);

-- Sync sequence for order_details table
SELECT setval('orderdetails_orderdetailid_seq', (SELECT MAX(orderdetailid) FROM OrderDetails) + 1);

-- Sync sequence for orders table
SELECT setval('orders_orderid_seq', (SELECT MAX(orderid) FROM orders) + 1);

-- Sync sequence for products table
SELECT setval('products_productid_seq', (SELECT MAX(productid) FROM products) + 1);

-- Sync sequence for shippers table
SELECT setval('shippers_shipperid_seq', (SELECT MAX(shipperid) FROM shippers) + 1);

-- Sync sequence for suppliers table
SELECT setval('suppliers_supplierid_seq', (SELECT MAX(supplierid) FROM suppliers) + 1);
